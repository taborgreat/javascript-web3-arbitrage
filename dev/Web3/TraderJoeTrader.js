...
async function performSwap() {
    ...
    const amountInWei = ethers.parseUnits(amountInEther.toString(), 18);
const approvalAmount = (BigInt(amountInWei) * BigInt(1000000)).toString();




    const path = [WAVAX_ADDRESS, '0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7'];
    ...
    // Approve the router to spend AVAX on behalf of the wallet
    const approveTx = await avaxContract.approve(ROUTER_ADDRESS, approvalAmount);
   await approveTx.wait();
    ...
    // Make sure to adjust the gas limit based on your requirements
    const gasLimit = 300000;

    //const deadline = Math.floor(Date.now() / 1000) + 300;
    const deadline = Math.floor(Date.now() / 1000) + 600;
    // Send AVAX along with the transaction as it requires a payable transaction
    const swapTx = await uniswapRouter.swapAVAXForExactTokens(
      amountInWei,
      path,
      walletAddress,
      deadline
